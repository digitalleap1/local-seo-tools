...
export async function runScheduleScan(s, { scanFn = runGridScan, reschedule = true, now = Date.now() } = {}){
  ...
  return { schedule:s.id, business:biz.name, keywords:list.length, done, errored };
}
